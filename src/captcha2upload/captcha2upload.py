...
class CaptchaUpload:
    ...
    def report(self,id):
         """
        This function return the captcha report
        :param id: id captcha returned by upload
        :return: <REPORT> OK | 1 ERROR!
        """
    ...
    def solve(self, pathfile):
        ...
        :return: [<id>, <captchaword>] OK | 1 ERROR!
        """
        if exists(pathfile):
            ...
            if request.ok:
                if request.text.split('|')[0] == "OK":
                    ...
                    return request.text.split('|')[1],self.getresult(request.text.split('|')[1])
